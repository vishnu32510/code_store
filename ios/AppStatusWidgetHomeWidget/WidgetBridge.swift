...
/// Universal communication bridge for decoding Flutter structured models and values in iOS WidgetKit extensions.
public struct WidgetBridge {
    ...
    public static func actionUrl(forKey key: String) -> URL? {
        guard let uriString = userDefaults?.string(forKey: "\(key)_action_uri") ?? userDefaults?.string(forKey: key),
